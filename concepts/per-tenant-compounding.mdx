...
graph TB
    ...
    subgraph OfflinePipeline["Offline Batched Pipeline"]
        direction LR
        Batch["Batch Processor"]
        Versioning["Version Controller"]
    end

    subgraph TenantModel["Tenant A Model"]
        direction LR
        ModelVN["Versioned Model vN"]
        ScoringEngine["Scoring Engine"]
    end

    TenantA --> OfflinePipeline
    OfflinePipeline --> TenantModel
    TenantModel --> Output["Tenant A Scoring Only"]

    TenantB["Tenant B Data"] -.->|No cross-tenant access| Output
```
